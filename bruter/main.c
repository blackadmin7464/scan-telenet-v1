...
volatile int processed = 0, failed = 0, processing = 0, found = 0,  maxfds = 0, last_found = 0, left_in_queue = 0, thread = 0;
...
void *writer_init()
{
    ...
    while(1)
    {
        printf("%ds, Processed: %d, Processing: %d, Failed: %d, Found: %d, Queued: %d, Last Find: %ds ago\n",
            ATOMIC_INC(&seconds), ATOMIC_GET(&processed), ATOMIC_GET(&processing) > 0 ? ATOMIC_GET(&processing) : 0, ATOMIC_GET(&failed), ATOMIC_GET(&found), ATOMIC_GET(&left_in_queue), ATOMIC_GET(&found) > 0 ? ATOMIC_INC(&last_found) : 0);
        sleep(1);
    }
}
